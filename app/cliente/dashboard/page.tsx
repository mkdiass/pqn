...
import { Activity, ArrowRight, CreditCard, FileText, Gauge, Headphones, Home, LifeBuoy, LogOut, ReceiptText, Settings, ShieldCheck, Wifi, Zap } from "lucide-react";
import { redirect } from "next/navigation";
import { getClientSession } from "@/lib/auth";
import { clientContract } from "@/data/client-portal";
...
function extractPlanSpeed(plan: string) {
  const match = plan.match(/(\d+(?:[.,]\d+)?)\s*(?:Mega|Mbps|Gbps)/i);
  if (!match) return null;
  const value = Number(match[1].replace(",", "."));
  if (/gbps/i.test(match[0])) return value * 1000;
  return value;
}
...
export default async function ClientDashboard() {
  ...
  const speed = extractPlanSpeed(session.plan) ?? extractPlanSpeed(clientContract.plan);
    ...
    <main className={styles.shell}>
      <aside className={styles.sidebar}>
        ...
        <nav className={styles.nav} aria-label="Navegação da Central do Cliente">
          ...
          <Link href="/cliente/suporte"><Headphones size={17}/><span>Suporte</span></Link>
      ...
      <div className={styles.main}>
        ...
        <div className={styles.container}>
          ...
          <section className={styles.metrics} id="performance" aria-label="Indicadores da conexão">
            <article className={styles.metric}><div className={styles.metricTop}><label>VELOCIDADE</label><div className={styles.metricIcon}><Zap size={16}/></div></div><strong>{speedLabel}</strong><span>Plano contratado</span></article>
            <article className={styles.metric}><div className={styles.metricTop}><label>DISPONIBILIDADE</label><div className={styles.metricIcon}><ShieldCheck size={16}/></div></div><strong>99,9%</strong><span>Indicador demonstrativo</span></article>
            <article className={styles.metric}><div className={styles.metricTop}><label>LATÊNCIA</label><div className={styles.metricIcon}><Activity size={16}/></div></div><strong>8 <small>ms</small></strong><span>Indicador demonstrativo</span></article>
          ...
          <div className={styles.contentGrid}>
            <section className={`${styles.card} ${styles.planCard}`}><div className={styles.cardHeader}><div><span>SEU SERVIÇO</span><h2>{session.plan}</h2></div><Link href="/cliente/contrato" className={styles.link}>Ver contrato <ArrowRight size={14}/></Link></div><div className={styles.planVisual}><div><div className={styles.planSpeed}>{speedLabel}</div><div className={styles.planName}>Fibra óptica Parque Net</div><div className={styles.planMeta}>Alta performance para streaming, trabalho e jogos.</div></div><div className={styles.planOrb} aria-hidden="true"><Wifi size={38}/></div></div><div className={styles.quality}><div className={styles.qualityHead}><span>Qualidade estimada da experiência</span><strong>Excelente</strong></div><div className={styles.bar}><div className={styles.fill}/></div></div></section>
            ...
            <section className={styles.card}><div className={styles.cardHeader}><div><span>ACESSO RÁPIDO</span><h2>Resolva em poucos cliques</h2></div></div><div className={styles.actionList}>
              ...
              <Link href="/cliente/suporte" className={styles.actionItem}><span className={styles.actionIcon}><LifeBuoy size={16}/></span><span><strong>Falar com suporte</strong><small>Abra uma solicitação de atendimento</small></span><ArrowRight size={15}/></Link>
            </div></section>
            ...
            <section className={`${styles.card} ${styles.support}`}><div><span className={styles.supportEyebrow}>PRECISA DE AJUDA?</span><h2>Estamos aqui.</h2><p>Abra uma solicitação autenticada e acompanhe o próximo passo com nossa equipe.</p></div><Link href="/cliente/suporte">Abrir suporte <ArrowRight size={15}/></Link></section>
          </div>
