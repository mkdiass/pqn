...
export function PlanCard({ speed, price, description, highlight = false, selected = false }: PlanCardProps) {
  return (
    <article className={`full-plan-card ${highlight ? "featured" : ""} ${selected ? "selected" : ""}`} id={`plano-${speed}`}>
      {highlight && <div className="full-plan-badge">MAIS ESCOLHIDO</div>}
      {selected && <div className="full-plan-badge">PLANO SELECIONADO</div>}

      <div className="full-plan-header">
        <div>
          <span className="full-plan-label">INTERNET FIBRA</span>
          <div className="full-plan-speed"><strong>{speed}</strong><span>MEGA</span></div>
        </div>
        <div className="full-plan-icon"><Wifi size={24} /></div>
      </div>
      ...
      <div className="full-plan-price"><span>R$</span><strong>{price}</strong><small>/mês</small></div>
      ...
      <Link href={`/cobertura?plano=${encodeURIComponent(speed)}`} className="full-plan-button">
