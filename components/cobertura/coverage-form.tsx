...
export function CoverageForm({ initialCep = "", autoSearchToken = 0 }: CoverageFormProps) {
  ...
  const searchCep = useCallback(async (cleanCep: string, shouldScroll = false) => {
    ...
    try {
      ...
      if (shouldScroll) {
        requestAnimationFrame(() => document.getElementById("coverage-consulta")?.scrollIntoView({ behavior: "smooth", block: "start" }));
      }
    } catch {
  ...
  function handleCoverageSearch() {
    ...
    setError("");
    const result = checkCoverage({ street: address.street, neighborhood: address.neighborhood, city: address.city, state: address.state });
    ...
    <>
      <section id="coverage-consulta" className="coverage-form-section">
        <div className="coverage-form-container">
          ...
          {selectedPlan && (
            <div className="pn-selected-plan">
              <div><strong>Plano selecionado: {selectedPlan.speed} Mega</strong><span>R$ {selectedPlan.price}/mês · você pode trocar de plano a qualquer momento.</span></div>
              <a href="/planos#planos">Trocar plano</a>
            </div>
          )}

          <div className="coverage-form-card">
            <div className="coverage-form-card-header">
              <div className="coverage-form-card-icon"><MapPin size={22} /></div>
              <div><h3>Qual é o seu endereço?</h3><p>Comece informando seu CEP.</p></div>
            </div>
            ...
            <div className="coverage-form">
              <div className="coverage-field coverage-cep-field">
                ...
                <div className="coverage-cep-input">
                  ...
                  <button type="button" onClick={handleSearchCep} disabled={loading || cep.replace(/\D/g, "").length !== 8}>
                    {loading ? <Loader2 size={19} className="coverage-loading" /> : <><Search size={18} />Buscar endereço</>}
                  </button>
                </div>
              ...
              {addressFound && (
                <div className="coverage-address-fields">
                  ...
                  <div className="coverage-field coverage-number-field"><label htmlFor="number">Número</label><input id="number" type="text" inputMode="numeric" placeholder="Ex.: 123" value={number} onChange={(event) => setNumber(event.target.value.replace(/\D/g, ""))} /></div>
                  ...
                  <button type="button" className="coverage-submit-button" onClick={handleCoverageSearch}>Consultar cobertura <ArrowRight size={19} /></button>
                </div>
              )}
            </div>
      ...
      {coverageChecked && (
        <div ref={resultRef}>
          <CoverageResult hasCoverage={hasCoverage} selectedPlan={selectedPlan} address={fullAddress} />
        </div>
      )}
    </>
  );
