...
const DEMO_PASSWORD_SALT = "pqn-demo-client-v1";
const DEMO_PASSWORD_HASH = "6b296184c23b5dfebaaad42c1d513dac3222baf7634b6bbec459bf424b8d19e7";
...
function verifyPassword(password: string) {
  const expected = Buffer.from(process.env.DEMO_CLIENT_PASSWORD_HASH || DEMO_PASSWORD_HASH, "hex");
  const received = scryptSync(password, DEMO_PASSWORD_SALT, expected.length, { N: 16384, r: 8, p: 1 });
  return received.length === expected.length && timingSafeEqual(received, expected);
}
export function getDemoCredentials() { return { email: process.env.DEMO_CLIENT_EMAIL || DEMO_CLIENT.email, password: "ParqueNet@2026" }; }
export function authenticateDemoClient(email: string, password: string) {
  const credentials = getDemoCredentials();
  if (email.trim().toLowerCase() !== credentials.email.toLowerCase() || !verifyPassword(password)) return null;
  return DEMO_CLIENT;
}
export async function createClientSession(session: ClientSession) {
  ...
  (await cookies()).set(SESSION_COOKIE, value, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", maxAge: SESSION_MAX_AGE, path: "/" });
}
